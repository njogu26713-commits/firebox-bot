...
function initAdminApi(app) {
    ...
    app.post("/api/bot/restart", authenticate, (req, res) => {
        ...
        setTimeout(() => {
            // Railway is configured with ON_FAILURE, so exit non-zero to
            // request a fresh container instead of leaving the bot offline.
            process.exit(1);
        }, 1500);
    });
    ...
    // 📷 QR Code — return the current QR as a base64 data URL
    app.get("/api/qr", async (req, res) => {
        if (!global.latestQr) {
            return res.status(404).json({ error: "No QR code available right now. The bot may already be connected or is still starting." });
        }
        try {
            const QRCode = require("qrcode");
            const dataUrl = await QRCode.toDataURL(global.latestQr, { width: 300, margin: 2 });
            return res.json({ qr: dataUrl });
        } catch (e) {
            return res.status(500).json({ error: "Failed to generate QR image: " + e.message });
        }
    });

    // 🧹 Force-wipe session — clear stale credentials before a fresh pairing
    app.post("/api/wipe-session", (req, res) => {
        try {
    ...
    // 🔑 Restore Session — decode a FIREBOX~ session ID and write creds.json, then restart
    app.post("/api/restore-session", (req, res) => {
        ...
        try {
            ...
            const encoded = rawId.includes("~") ? rawId.split("~")[1] : (rawId.startsWith("Firebox") ? rawId.slice(5) : rawId);
            ...
            // Find the first valid JSON block containing credentials
            let finalJson = null;
            ...
            if (!finalJson) {
                return res.status(400).json({ error: "Could not find valid credentials in that Session ID. Make sure you copied the full FIREBOX~ string." });
            }

            const parsed = JSON.parse(finalJson);
            const creds = parsed.creds || (parsed.noiseKey ? parsed : null);
            if (!creds) {
                return res.status(400).json({ error: "Session ID decoded but no credentials found inside. It may be corrupt or from a different bot." });
            }
            ...
            console.log("✅ [Dashboard] Session ID restored via web UI. Restarting bot...");
            res.json({ success: true, message: "Session restored! Bot is restarting now — this page will reload automatically." });
            setTimeout(() => process.exit(1), 1500); // exit(1) triggers ON_FAILURE restart on Railway
        } catch (e) {
