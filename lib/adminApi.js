...
function initAdminApi(app) {
    ...
    // 📱 Pairing Code — generate a WhatsApp link code for a given phone number
    app.post("/api/pair-code", async (req, res) => {
        ...
        const sock = global.sock;
        if (!sock) {
            return res.status(503).json({ error: "Bot socket not initialised yet. Try again in a few seconds." });
        }
